...
const copy = {
  ar: {
    ...
    androidInstallSteps: ["حمّل APK التجريبي من زر التنزيل على هاتف Android، ثم افتح ملف التنزيل.", "اسمح مؤقتًا للتثبيت من المتصفح أو مدير الملفات عندما يطلب Android ذلك، ثم ثبّت التطبيق.", "افتح عيالنا وأنشئ PIN للوالد، ثم فعّل البصمة إن كانت متاحة.", "افتح إدارة الأطفال لرؤية الأطفال المعرّفين، اختر الطفل النشط، أو اضغط إضافة طفل. حدّد ملفه العمري ثم راجع حدود التواصل والألعاب والنوم والصلاة.", "إعداد الجدول مفعّل افتراضياً من 09:00 إلى 21:00 كل يوم. راجع الأوقات والأيام أو أوقفه من لوحة الوالدين إذا لزم.", "من مهام وتذكيرات الطفل سجّل ملاحظة بصوت الوالدين، ثم اختر التكرار كل ساعة أو كل ساعتين. يفتح الإشعار عيالنا لتشغيل الصوت محليًا؛ لا يعمل تلقائيًا في الخلفية.", "امنح Usage Access وAccessibility وOverlay. فعّل Device Admin فقط إذا أردت محاولة القفل الصارم؛ يبقى الحاجب المرئي بديلًا عند عدم توفره.", "صنّف التطبيقات المثبتة، وراجع سجل الفئات قبل تسليم الجهاز للطفل."],
  ...
  en: {
    ...
    androidInstallSteps: ["Download the evaluation APK using this page’s download button on an Android phone, then open the downloaded file.", "Temporarily allow installation from your browser or file manager when Android asks, then install the app.", "Open 3ialna, create a parent PIN, and enable biometrics where available.", "Open Kids management to list configured children, select the active child, or choose Add child. Select an age profile, then review social, games, sleep, and prayer settings.", "Schedule settings are enabled by default from 09:00–21:00 every day. Review the times and days, or turn the schedule off from the parent dashboard if needed.", "In Child tasks and reminders, record a parent voice note and choose every hour or every two hours. The notification opens 3ialna to play the note locally; it does not autoplay in the background.", "Grant Usage Access, Accessibility, and Overlay. Enable Device Admin only if you want to attempt hard lock; the visual blocker remains the fallback when it is unavailable.", "Categorize installed apps, then review the category rules before handing the device to a child."],
