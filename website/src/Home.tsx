  ...
  return (
    <div dir={isArabic ? "rtl" : "ltr"} className="site-shell">
      ...
      <main id="top">
        ...
        <section id="faq" className="faq-section section-grid"><div className="faq-intro"><div className="editorial-stamp" aria-hidden="true"><span className="stamp-seed" /><span><b>3ialna</b><small>FAQ · HELP GUIDE</small></span></div><span className="english-rail-label">FAQ / TROUBLESHOOTING</span><span className="section-kicker">08 / FAQ</span><h2>{t.faq}</h2><CircleHelp size={34} className="faq-mark" /></div><div className="faq-list">{t.faqItems.map(([question, answer], index) => <div className={`faq-item ${openFaq === index ? "open" : ""}`} key={question}><button type="button" id={`faq-question-${index}`} aria-expanded={openFaq === index} aria-controls={`faq-answer-${index}`} onClick={() => setOpenFaq(openFaq === index ? null : index)}><span>{question}</span>{openFaq === index ? <ArrowUp size={18} aria-hidden="true" /> : <ArrowDown size={18} aria-hidden="true" />}</button><p id={`faq-answer-${index}`} role="region" aria-labelledby={`faq-question-${index}`} hidden={openFaq !== index}>{answer}</p></div>)}</div></section>
