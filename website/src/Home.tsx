...
import { ArrowDown, ArrowUp, ArrowUpRight, Check, ChevronDown, CircleHelp, Clock3, Download, ExternalLink, Globe2, LoaderCircle, LockKeyhole, Mail, MessageCircle, Mic2, RotateCcw, Send, Share2, ShieldCheck, Smartphone, Sparkles } from "lucide-react";
...
const copy = {
  ar: {
    ...
    faq: "أسئلة شائعة",
    faqSupportBody: "لم تجد ما تبحث عنه؟ اكتب لنا وسنساعدك في الإعداد.",
    ...
    testerQrTitle: "امسح لتنزيل النسخة المستقرة",
    testerQrBody: "يفتح تنزيل arm64 المباشر على هاتف Android دون تسجيل دخول.",
  ...
  en: {
    ...
    faqSupportBody: "Still need a hand? Send us a message and we’ll help with setup.",
    ...
    testerQrTitle: "Scan to download the stable APK",
    testerQrBody: "Opens the direct arm64 Android download without sign-in.",
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const faqSchema = { "@context": "https://schema.org", "@type": "FAQPage", "inLanguage": language, "mainEntity": t.faqItems.map(([question, answer]) => ({ "@type": "Question", name: question, acceptedAnswer: { "@type": "Answer", text: answer } })) };
    let schema = document.getElementById('3ialna-faq-schema') as HTMLScriptElement | null;
    if (!schema) { schema = document.createElement("script"); schema.id = "3ialna-faq-schema"; schema.type = "application/ld+json"; document.head.appendChild(schema); }
  ...
  const telegramShareUrl = useMemo(() => `https://t.me/share/url?url=${encodeURIComponent(preferredDownloadUrl)}&text=${encodeURIComponent(shareText)}`, [shareText, preferredDownloadUrl]);
  ...
  return (
    <div dir={isArabic ? "rtl" : "ltr"} className="site-shell">
      ...
      <main id="top">
        ...
        <section id="faq" className="faq-section section-grid"><div className="faq-intro"><div className="editorial-stamp" aria-hidden="true"><span className="stamp-seed" /><span><b>3ialna</b><small>FAQ · HELP GUIDE</small></span></div><span className="english-rail-label">FAQ / TROUBLESHOOTING</span><span className="section-kicker">08 / FAQ</span><h2>{t.faq}</h2><CircleHelp size={34} className="faq-mark" /></div><div className="faq-list">{t.faqItems.map(([question, answer], index) => { const isOpen = openFaq === index; const isClosing = closingFaq === index; return <div className={`faq-item ${isOpen ? "open" : ""}`} key={question}><button type="button" id={`faq-question-${index}`} aria-expanded={isOpen} aria-controls={`faq-answer-${index}`} onClick={() => { const reduceMotion = window.matchMedia?.("(prefers-reduced-motion: reduce)").matches; if (isOpen) { setOpenFaq(null); setClosingFaq(reduceMotion ? null : index); } else { setClosingFaq(reduceMotion || openFaq === null ? null : openFaq); setOpenFaq(index); } }}><span>{question}</span>{isOpen ? <ArrowUp size={18} aria-hidden="true" /> : <ArrowDown size={18} aria-hidden="true" />}</button><div className={`faq-answer-shell ${isOpen ? "is-open" : isClosing ? "is-closing" : ""}`} onTransitionEnd={(event) => { if (event.propertyName === "grid-template-rows" && closingFaq === index) setClosingFaq(null); }}><p id={`faq-answer-${index}`} role="region" aria-labelledby={`faq-question-${index}`} aria-hidden={!isOpen} hidden={!isOpen && !isClosing}>{answer}</p></div></div>; })}</div><div className="faq-support"><p>{t.faqSupportBody}</p><a className="faq-support-button" href="#contact"><MessageCircle size={17} aria-hidden="true" />{t.contactSupport}<ArrowUpRight size={16} aria-hidden="true" /></a></div></section>
